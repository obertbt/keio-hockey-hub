...
## Phase 4: 日報・トレーニング（次にやる）

テーブルと RLS は作成済み。残っているのは画面と Server Action。
...
- [ ] 練習前コンディションの入力画面
- [ ] 今日の個人目標の入力画面
- [ ] 日報の入力画面（下書き保存、公開範囲の既定は staff）
- [ ] トレーニング記録の入力画面（種別ごとに項目が変わる）
- [ ] 自主練動画からフィードバック依頼を作る導線
- [ ] コーチ向け提出状況の一覧
- [ ] 日報へのコーチコメント
- [ ] 過去の日報の閲覧
...
- [x] 単体テスト
- [ ] Upload Session の作成 API
- [ ] Presigned PUT URL の発行
- [ ] ブラウザからの直接アップロード
- [ ] アップロード後の実物確認
- [ ] Signed GET URL の発行
- [ ] 動画メタデータの記録
...
- [ ] スキル一覧・進捗表示
- [ ] スキル申請
- [ ] 根拠（動画・クリップ・フィードバック）の添付
- [ ] コーチの審査
- [ ] 承認と履歴
- [ ] フィードバック回答からスキル申請への導線
...
- [x] E2E: 選手の1日（Supabase 必要）
- [ ] E2E: 動画の流れ（Phase 5-6 の後）
- [ ] E2E: R2 アップロード（Phase 7 の後）
