...
### `private` にすると提出状況にも出ない

RLS は行単位で効くため、「行があることは見せるが、中身は見せない」が書けません。
そのため公開範囲を `private` にした日報は、
コーチの提出状況の画面でも **未提出として表示されます**。

今のところ意図した挙動です。「自分だけ」を選んだ以上、
コーチ側には存在も伝わらないほうが、約束として一貫しているためです。

ただし運用上は「出したのに催促される」ことが起こり得ます。
提出の事実だけを別に持つか、ビューを作るかを検討課題として残しています
（TASKS.md の「積み残し」）。
